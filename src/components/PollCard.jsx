...
export default function PollCard({ poll, remaining, isFinished, mode }) {
  ...
  const getCategoryColor = (category) => {
    const colors = {
      ...
      "Musique": "bg-purple-100 text-purple-600",
      "Films": "bg-red-100 text-red-600",
      "Jeux vidéo": "bg-amber-100 text-amber-600",
      "Éducation": "bg-indigo-100 text-indigo-600",
      "Nourriture": "bg-emerald-100 text-emerald-600",
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <div className="flex items-center gap-1.5 text-gray-500">
          ...
          <span className="text-sm font-medium">
            {formatVoters(poll.voters || 0)} votant{poll.voters !== 1 ? 's' : ''}
          </span>
        </div>

        {poll.categorie && (
          <div className={`px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1.5 ${getCategoryColor(poll.categorie)}`}>
