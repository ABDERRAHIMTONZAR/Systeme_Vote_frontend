  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        ...
        const statsRes = await axios.get("http://localhost:3001/dashboard/stats", { headers });
        setStats(statsRes.data);

        const monthlyRes = await axios.get("http://localhost:3001/dashboard/monthly-stats", { headers });
        setChartData(monthlyRes.data);

        const pollStatusRes = await axios.get("http://localhost:3001/dashboard/poll-status", { headers });
        setPollStatus({
          active: Number(pollStatusRes.data.active),
          finished: Number(pollStatusRes.data.finished)
        });

        const engagementRes = await axios.get("http://localhost:3001/dashboard/engagement", { headers });
